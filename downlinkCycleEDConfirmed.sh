...
# echo "Done initialization"
    ...
    do
        ...
        output="$(./waf --run "RawCompleteNetworkPerformances
            ...
            --maxNumbTx=$maxNumbTx
            --DRAdapt=$DRAdapt
            --propAckToImprovement=$ackToImprov
            --subBandPriorityImprovement=$subBandPriorImprov
            --RW2DataRateImprovement=$rw2DataRateImprov
            --RngRun=$globalrun" | grep -v "build" | tr -d '\n')"
         echo "$output"

         # "Devices appPeriodseconds totalPacketsSent totalReceivedPackets totalInterferedPackets totalNoMoreReceiversPackets totalUnderSensitivityPackets *packetLostBecauseTx* s1 s2 s3 s4 s5 s6 s7 s8 f1 f2 f3 f4 f5 f6 f7 f8 averageDelay averageAckDelay totalRetxAmounts \n nSF7 nSF8 nSF9 nSF10 nSF11 nSF12 nOOR"
