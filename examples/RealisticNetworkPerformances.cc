...
NS_LOG_COMPONENT_DEFINE ("RealisticNetworkPerformances");
...
int main (int argc, char *argv[])
{
  ...
  // Set up logging
  LogComponentEnable ("RealisticNetworkPerformances", LOG_LEVEL_ALL);
  ...
  ************************/
  ...
  int percConfirmed = floor(0.3*nDevices);
  int device = 0;

  // Connect trace sources
  // and set confirmed messages only for 30% of the devices
  for (NodeContainer::Iterator j = endDevices.Begin ();
       j != endDevices.End (); ++j)
    {
      Ptr<Node> node = *j;
